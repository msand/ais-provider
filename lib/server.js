...
const extractDate = (fileName) =>  fileName.replace(/^(\d{4}-\d{2}-\d{2}).jol.gz$/, '$1')

app.get('/vessel/path/:imo/:name/:date', (req, res) => {
  const filePath = store.securelyResolveFileName(path.join(req.params.imo, req.params.name, req.params.date) + '.jol.gz')
  console.log('ABOUT TO OPEN FILE', filePath)
