    ...
    request(url, function (err, response, body) {
      if(err) {
        ...
        parse(body, {
          ...
          if(err) {
            console.error('Failed to parse CSV when fetching data from URL:', url)
            console.error(err)
            callback()
          } else if(!aisEntries || aisEntries.length === 0) {
            console.error('No data found when parsing CSV from URL:', url)
            callback()
          } else {
            ...
            aisEntries.forEach(aisEntry => {
              ...
              if(aisEntry.mmsi && aisEntry.imo && aisEntry.imo > 1000000 && aisEntry.imo < 10000000) {
                validVesselCount ++
                const vessel = {
                  mmsi: parseInt(aisEntry.mmsi, 10),
                  imo: parseInt(aisEntry.imo, 10),
            ...
            // console.info('vessels imported / vessel received', validVesselCount, '/', aisEntries.length)
            // callback()
          }
        })
      } else {
